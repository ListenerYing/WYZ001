...
export default {
  ...
  methods: {
    // 获取时间配置数据的方法
    load() {
      this.request.get(`/deadline/page?pageNum=${this.query.pageNum}&pageSize=${this.query.pageSize}`)
          .then(res => {
            if (res.data) {
              this.deadlines = res.data.records;
    ...
    handleAdd() {
      ...
      this.form = { id: null, description: '', startTime: '', endTime: '' };
    ...
    save(){
      this.request.post("/deadline",this.form).then(res=> {
        if(res.data){
    ...
    // 保存时间配置
    handleSave() {
      const url = this.currentAction === 'create' ? '/deadline' : `/deadline/${this.form.id}`;
    ...
    handleDelete(id) {
      this.$confirm('确认删除这个时间配置吗?', '删除确认', {
        ...
        this.request.delete(`/deadline/${id}`).then(res => {
          if (res.data) {
    ...
    toggleActive(row) {
        ...
        // 然后创建更新的Deadline对象，确保反转isActive值
        ...
        // 发送 POST 请求到后端来保存更新的 Deadline 对象
      this.request.post(`/deadline`, updatedDeadline).then(res => {
        if (res.data) {
